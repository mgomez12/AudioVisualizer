...
class VertexObject {
 public:
  ...
  void UpdateLifespans(std::unique_ptr<LifespanArray> lifespans);
  ...
  bool HasLifespans() const {
    return lifespans_ != nullptr;
  }
  ...
  const LifespanArray& GetLifespans() const {
    if (lifespans_ == nullptr)
      throw std::runtime_error("No indices in VertexObject!");
    return *lifespans_;
  }
 ...
 private:
  ...
  std::unique_ptr<LifespanArray> lifespans_;
};
