...
void VertexArray::CreateLifespanBuffer() {
  lifespan_buf_ = make_unique<LifespanBuffer>(GL_STATIC_DRAW);
}
...
void VertexArray::UpdateLifespans(const LifespanArray& lifespans) const {
  lifespan_buf_->Update(lifespans);
}
...
void VertexArray::LinkLifespanBuffer(GLuint attr_idx) const {
  BindGuard vao_bg(this);
  BindGuard buf_bg(lifespan_buf_.get());
  // The line below attaches the vertex buffer to the VAO.
  GL_CHECK(glVertexAttribPointer(attr_idx, 1, GL_FLOAT, GL_FALSE, 0, 0));
  GL_CHECK(glEnableVertexAttribArray(attr_idx));
}
...
void VertexArray::Render(size_t start_index, size_t num_indices) const {
  ...
  GLint draw_mode = draw_mode_ == DrawMode::Triangles ? GL_TRIANGLES : GL_POINTS;
