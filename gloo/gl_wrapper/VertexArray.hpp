...
class VertexArray : public IBindable {
 public:
  ...
  void UpdateLifespans(const LifespanArray& lifespans) const;
  ...
  void LinkLifespanBuffer(GLuint attr_idx) const;
  ...
  bool HasLifespanBuffer() const {
    return lifespan_buf_ != nullptr;
  }
 ...
 private:
  ...
  using LifespanBuffer = VertexBuffer<float, GL_ARRAY_BUFFER>;
  ...
  std::unique_ptr<LifespanBuffer> lifespan_buf_;
