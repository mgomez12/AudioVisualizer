...
void VertexObject::UpdateLifespans(std::unique_ptr<LifespanArray> colors) {
  if (lifespans_ == nullptr) {
    vertex_array_->CreateLifespanBuffer();
  }
  lifespans_ = std::move(colors);
  vertex_array_->UpdateLifespans(*lifespans_);
}
